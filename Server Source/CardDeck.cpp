...
CardDeck::CardDeck()
{
	...
	/*while (true)
	{
		...
		std::string result;
		CalculateHandStrength(cards, result);
	}*/
}
	...
	//
		...
		for (uint8 i = 1; i <= MAX_VALUE; ++i)
		{
			...
			if (vNumOfEachValue[i] == 2)
			{
				...
				#define ONE_PAIR_FORMULA ONE_PAIR + (50000 * i)
			...
			if (vNumOfEachValue[i] == 3)
			{
				...
				#define THREE_OF_KIND_FORMULA THREE_OF_KIND + (50000 * i)
